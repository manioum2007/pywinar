...
class Sfx(wx.Dialog):
    def __init__(self,parant,titre):
        wx.Dialog.__init__(self,parant,-1,titre)
        
        self.sizer = wx.BoxSizer(wx.VERTICAL)
        self.panel=wx.Panel(self,-1)
        self.notb = fnb.FlatNotebook(self.panel)
        hbx= wx.BoxSizer(wx.HORIZONTAL)
        ii=Information(self.notb)
        
        self.notb.AddPage(ii, "Information")
        
        
        cc=Commentaire(self.notb)
        self.notb.AddPage(cc, "Commentaire")
        
        ss=sfx(self.notb)
        self.notb.AddPage(ss, "Sfx")
